...
const AccordionSelect = ({ onGeocodeSelect }) => {
  ...
  useEffect(() => {
    if (selectedItems.length === 0) {
      ...
      const abbreviatedNames = filteredNames.map((name) => {
      ...
      const displayedNames =
        abbreviatedNames.length > 4
          ? [...abbreviatedNames.slice(0, 4), "…"]
          : abbreviatedNames;

      setPlaceholderText(displayedNames.join(", "));
    }
  }, [selectedItems]);
    ...
    <View style={{ flex: 1, marginRight: 10 }}>
      <TouchableOpacity onPress={toggleAccordion} style={styles.rowContainer}>
        ...
        <FontAwesome
          ...
          style={styles.icon}
          onPress={() => {
            console.log("Lupa foi clicada");
            onGeocodeSelect(selectedItems); // Envia os geocodes
            console.log("Função de foco foi chamada");
          }}
        />
      </TouchableOpacity>
